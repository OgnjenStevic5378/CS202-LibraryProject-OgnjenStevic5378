...
public class DataBase {
    ...
    public static void addNewClient(String name, String lastName, Date birthDate) { // The SQL Date, not regular one
        try {
            connection = DriverManager.getConnection(url, username, password);
    ...
    public static void addNewCD(String name, String genre, Integer publisherYear) { // publisherYear is int
        try {
            connection = DriverManager.getConnection(url, username, password);
    ...
    public static ArrayList<StatisticData> getStatisticData() {
        ...
        try {
            connection = DriverManager.getConnection(url, username, password);
            String query =
                    "SELECT cd_and_client.id_cd_and_client, client.id_client, CONCAT(client.name, ' ', client.last_name) AS full_name, cd.id_cd, cd.name AS cd_name, cd_and_client.begin_date, cd_and_client.end_date FROM cd_and_client\n" +
                            "INNER JOIN client ON cd_and_client.fk_client = client.id_client\n" +
                            "INNER JOIN cd ON cd_and_client.fk_cd = cd.id_cd";
            Statement st = (Statement) connection.createStatement();
            ResultSet rs = st.executeQuery(query);
            while (rs.next()) {
                int idCDAndClient = rs.getInt("id_cd_and_client");
    ...
    public static void issueCD(int idCD, int idClient, Date currentDate) {
        try {
            connection = DriverManager.getConnection(url, username, password);
            ...
            PreparedStatement preparedStatement = connection.prepareStatement("INSERT INTO cd_and_client (id_cd_and_client, fk_cd, fk_client, begin_date, end_date) "
                    + "VALUES (NULL, ?, ?, ?, NULL)");
            preparedStatement.setInt(1, idCD);
            ...
            // Alert to show that we have an error
            if (e.toString().contains("FOREIGN KEY (`fk_client`) REFERENCES `client` (`id_client`)")) // The exception used for invalid Client ID
